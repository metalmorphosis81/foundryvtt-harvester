...
export default class ItemPilesHelpers {
    ...
    static stackTableResults(rolledResult) {
        ...
        rolledResult.forEach((newResult) => {
            let isResultHidden = foundry.utils.getProperty(newResult, `flags.better-rolltables.brt-hidden-table`) || false;
            // MOD 4535992
    ...
    static async unlinkToken(token) {
        ...
        if (tokenTarget instanceof foundry.canvas.placeables.Token) {
            await tokenTarget.document.update({ actorLink: false });
        } else if (tokenTarget instanceof foundry.documents.TokenDocument) {
